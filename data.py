...
def truncate_llama2(dataset, data, postfix_text=" ... [the rest of the text is omitted]"):
    ...
    def truncate(sample):
        ...
        return sample
    return data.map(truncate, num_proc=4)


def filter_length(data, min_length, key):
    tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-hf")
    data = data.filter(lambda x: len(tokenizer(x[key])['input_ids']) >= min_length, num_proc=4)
    return data
...
def load_icl(dataset, max_test_sample=None, seed=42):
    ...
    final_data = test_data.map(preprocess, num_proc=4)
